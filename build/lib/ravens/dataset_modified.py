...
class Dataset:
  ...
  def add(self, seed, episode):
    ...
    """
    color, depth, arm, object_pose, tar_pose = [], [], [], [], []
    for obs, act, pose, tar_pos in episode:
#      img = np.uint8(obs['color'][0])
#      d = np.float32(obs['depth'][0])
#      print(type(img))
#      print(type(img[0]))
#      img = cv2.resize(img, dsize=(160, 120), interpolation=cv2.INTER_CUBIC)
#      d = cv2.resize(d, dsize=(160, 120), interpolation=cv2.INTER_CUBIC)
#      color.append(img)
#      depth.append(d)
    ...
    dump(depth, 'depth')
    dump(arm, 'ee')
    dump(object_pose, 'object_pos')
    dump(tar_pose, 'targ_pos')

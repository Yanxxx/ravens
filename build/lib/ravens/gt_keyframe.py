...
#  agent = NaiveMDP()
  key_agent = NaiveMDP(naive=False)
...
#      opline = getline(actions)
      env.add_line(line)
#      print(opline)
#      env.add_line(opline, [0,1,1], 5)
      ...
      while not key_agent.done():
          ...
          for count, trace in enumerate(subtraces):
              ...
              for a in preced:
                  ...
                  print(r)            
              obs, r, done, info = env.step_move(cur[0])
              print(r + rp)
              reward.append(r + rp)
              for c, act in enumerate(trace):
          ...
          if len(subtraces) < 1:  
              ...
              reward.append(r + rp)
              rewards.append(reward)
              print('rewards no subtrace ', rewards)
